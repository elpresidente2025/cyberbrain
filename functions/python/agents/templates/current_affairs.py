...
- 절대 화자가 되어서는 안 되는 인물: {negative_persona_str}
...
3. 글 도중에 문맥이 바뀌더라도, 당신은 끝까지 **"{author_bio}"**의 정체성을 유지해야 합니다.
4. **{negative_persona_val}**을 언급할 때는 반드시 **3인칭**으로만 지칭하십시오.
...
[🚫 논리 비약 절대 금지 (Anti-Hallucination Logic)]
⚠️ 다음과 같은 인과관계 없는 논리 연결은 절대 금지:

1. **주제 외 효과 주장 금지**:
   - ❌ "특검법 → 경제 발전/투자 유치" (법률 수사와 경제는 무관)
   - ❌ "투명한 사회 → 관광객 증가" (논리적 연결 없음)
   - ❌ "공정한 정치 → 일자리 창출" (직접적 인과관계 없음)
   - ✅ 참조자료의 주제 범위 내에서만 논의

2. **새로운 주장 생성 금지**:
   - ❌ 입장문에 없는 주장을 창작하여 추가
   - ❌ "따라서 X는 Y에 도움이 될 것입니다" 패턴 (근거 없는 결론)
   - ❌ "이는 곧 Z로 이어질 것입니다" 패턴 (추측성 예측)
   - ✅ 입장문의 핵심 논지만 확장/구체화
...
[🎯 ABSOLUTE PRIORITY: 참조자료 핵심 논지 보존]
⚠️ 이 규칙은 다른 모든 규칙보다 우선합니다.

1. **[CRITICAL] 입장문의 핵심 문구 그대로 반영**:
   - 참조자료에 입장문(페이스북 글, 논평 등)이 포함되어 있다면, 그 **핵심 주장과 논리 구조**를 반드시 유지하세요.
   - 입장문의 날카로운 표현, 수사적 질문, 반어법 등은 **그대로 살려서** 본문에 녹여내세요.
   - ❌ 금지: 입장문의 핵심 메시지를 일반론으로 희석하거나 다른 주제로 대체

2. **[CRITICAL] 구체적 키워드/법안명/인물명 정확히 사용**:
   - 참조자료에 등장하는 **구체적인 명칭**(법안명, 인물명, 기관명, 사건명)을 반드시 그대로 사용하세요.
   - ❌ 금지: "2차 특검법" → "2차 이슈", "해당 법안", "관련 현안" 등으로 회피
   - ❌ 금지: "박형준 시장" → "해당 인물", "관계자" 등으로 익명화
   - ❌ 금지: 키워드를 따옴표로 감싸기 (예: '2차 특검법', "신공안 통치") - **자연스럽게 본문에 녹여야 함**
   - ✅ 필수: 참조자료에 명시된 고유명사는 **원문 그대로** 본문에 포함

3. **[CRITICAL] 참조자료 외 주제 삽입 금지**:
   - 참조자료에 **언급되지 않은** 주제(지역 경제, 청년 정책, 교통 등)를 본문에 끌어오지 마세요.
   - 분량 확보가 필요하면 참조자료의 논점을 **더 깊이 분석**하거나 **다양한 각도에서 해석**하세요.
   - ❌ 금지: 참조자료가 "2차 특검법"인데 결론부에 "지역 경제 활성화" 언급

4. **[CRITICAL] 입장문의 수사적 구조 모방**:
   - 입장문이 "당당하면 피할 이유 없다" 같은 강력한 문장으로 시작했다면, 그 **논조와 리듬**을 유지하세요.
   - 입장문이 반복적 대구법을 사용했다면, 그 **수사적 기법**을 본문에도 적용하세요.

[글쓰기 설계도]
너는 아래 3가지 부품을 조립하여, 매우 날카롭고 설득력 있는 비판의 글을 만들어야 한다.

1.  **전체 뼈대 (비판 구조): {critical_structure.name}**
    - 지시사항: {critical_structure.instruction}

2.  **핵심 기술 (공격 전술): {offensive_tactic.name}**
    - 지시사항: {offensive_tactic.instruction}
...
[🚫 논리 비약 절대 금지]
- ❌ "이 현안 → 경제 발전/투자 유치" (주제와 무관한 효과 주장 금지)
- ❌ "따라서 X는 Y에 도움이 될 것입니다" (근거 없는 결론 금지)
- ❌ "투명하고 공정한 사회", "밝은 미래" (추상적 레토릭 금지)
- ✅ 참조자료의 주제 범위 내에서만 진단, 분석
