...
def _h2_content_tokens(text: str) -> List[str]:
    ...
    kiwi 명사 추출(NNG/NNP/NNB)을 사용한다. kiwi 를 사용할 수 없는 환경이면
    RuntimeError 를 발생시킨다 — regex fallback 은 조사 포함 eojeol 을 그대로
    추출해 프로덕션과 다른 matching 결과를 낸다.
    """
    plain = str(text or "").strip()
    if not plain:
        return []
    kiwi_nouns = korean_morph.extract_nouns(plain)
    if kiwi_nouns is None:
        raise RuntimeError(
            "_h2_content_tokens: kiwi 를 사용할 수 없습니다. "
            "body_alignment 스코어링은 kiwi 형태소 분석기가 필수입니다."
        )
