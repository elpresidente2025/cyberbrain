...
def _build_aeo_rules() -> str:
  ...
  <banned>
    ...
    1인칭 표현("저는", "제가", "나는", "내가"),
    본문 고유 실행수단 없이 추상 틀만 반복하는 제목("실행 계획을 세우겠습니다", "제도 기반을 세우겠습니다", "제도로 뒷받침하겠습니다", "재정 우려, 어떻게 넘을까"),
    상투적 결론("미래를 열다", "새로운 도약", "함께 만들어가다", "변화를 이끌다", "밝은 전망") — 특히 마지막 H2에서 구체적 내용 없이 "혁신/미래/포용/도약/비전/발전" 등 추상어만으로 구성된 소제목은 hard-fail
  </banned>

  <aeo_rule>H2 바로 아래 첫 문장(40~60자)은 해당 소제목(약속)에 대한 직접 답변으로 작성할 것.</aeo_rule>
  <section_anchor_rule severity="critical">H2는 아래 본문에서 실제로 다루는 사업명·시설·절차·수치·대상 중 최소 1개를 반영한다. 주 검색어만 반복하고 "실행 계획/제도 기반/재정 우려" 같은 틀로 끝내면 실패다.</section_anchor_rule>
</h2_rules>"""
...
<h2_examples name="소제목 교정 예시 (bad → good)">
  ...
  <checklist>
    ...
    <ban>마지막 H2를 "혁신/미래/포용/도약/비전/발전" 같은 추상어만으로 구성 — 결론 H2에도 구체적 사업명·숫자·고유명사를 반드시 포함할 것. BAD: "혁신과 변화로 미래를 열겠습니다" → GOOD: "귤현동 탄약고 이전, 2027년까지 완료 목표"</ban>
  </checklist>
</h2_examples>
"""
