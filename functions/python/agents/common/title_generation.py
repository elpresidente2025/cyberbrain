...
<rules priority="critical">
  ...
  <rule id="no_demonstrative_abstract_title" priority="critical">제목에는 "이러한/이런/그러한/그런 + 정책/방안/사례/노력/가치/결과/변화/효과" 같은 지시 관형사 접착 표현을 쓰지 말 것. 제목은 앞 문맥이 없으므로 "이러한 정책"처럼 받지 말고, 정책명·수치·쟁점 자체를 직접 써야 한다.</rule>
  ...
  <rule id="no_broken_sentence" priority="critical">제목은 그 자체로 완결된 문장이어야 한다. 본문의 긴 문장을 제목 길이로 압축하면서 목적어·주어·부사어 등 필수 성분을 빠뜨리지 마라. 특히 타동사("구성하다/추진하다/마련하다/확보하다")를 쓸 때 "무엇을" 구성·추진·마련·확보하는지 반드시 명시할 것. 출력 전에 "주어-목적어-서술어가 모두 있는가?" 를 체크하고, 빠진 성분이 있으면 문장을 다시 쓰라.
    ❌ "[인물명]이 구성하고 관련 조례로 부활 모색" → "구성하고"의 목적어 없음
    ✅ "[키워드], [대상] 구성하고 조례 개정으로 부활 모색" → 목적어 명시</rule>
  ...
  <rule id="no_hollow_question_tail" priority="critical">제목 끝을 **"[추상명사][은/는]?"** 형태로 닫지 말 것. 예: "책임은?", "선택은?", "미래는?", "해답은?", "비결은?", "답은?", "방향은?", "뜻은?". 이것은 형식만 질문이고 내용이 공허한 안티패턴이다 — 독자가 "본인이겠지/당연하지"로 즉시 닫아버려 info_gap 이 발생하지 않는다. 반드시 다음 중 하나로 대체한다:
    - **의문사 기반 실질 질문**: "왜 ~인가", "어떻게 ~하는가", "얼마까지", "무엇이 달라졌나", "어디서 시작됐나" — 질문 뒤에 구체 재료가 붙어 있어야 함. "할까?/될까?/을까?" 같은 수사적 반문 어미는 사용 금지(답변 엔진이 매칭 못함).
  ...
  <rule id="hook_must_have_tension" priority="critical">&lt;hook_quality_rubric&gt; 4 차원 중 concrete_slot + specificity 조합만으로는 "턱걸이" 다 — 재료만 나열한 평탄체가 된다. 반드시 info_gap 또는 narrative_arc 중 최소 하나를 추가로 채워라.
    - info_gap 달성 방법: 물음표는 필수가 아니다. "왜 ~인가", "어떻게 ~하는가", "얼마나/얼마까지", "무엇이 달라졌나" 같은 **의문사 기반 실질 질문**이 기본이다. 질문 자체가 구체 재료를 물고 있어야 한다. "할까?/될까?" 같은 반문 어미는 금지.
