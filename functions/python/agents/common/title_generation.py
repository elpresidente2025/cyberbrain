...
TITLE_TYPES = {
    'VIRAL_HOOK': {
        ...
        'bad': [
            ...
            {'title': '이재성 부산 지방선거, AI 3대 강국?', 'problem': '키워드 나열 — 문장이 아님, 의미 불분명', 'fix': 'AI 국비 전패한 부산, 이재성이 꺼낸 카드는'},
            {'title': '결국 터질 게 터졌습니다... 충격적 현실', 'problem': '낚시 자극 — 구체성 제로, 신뢰 파괴', 'fix': '부산 경제 0.7%, 왜 이 남자가 뛰어들었나'}
        ]
    },
    'DATA_BASED': {
        'id': 'DATA_BASED',
...
<rules priority="critical">
  ...
  <rule id="keyword_position">핵심 키워드 "{primary_kw_str}" 반드시 포함 (자연스럽게)</rule>
  <rule id="no_greeting">인사말("안녕하세요"), 서술형 어미("~입니다") 절대 금지</rule>
  <rule id="style_ban">"발표", "개최", "참석" 등 보도자료 스타일 금지</rule>
  <rule id="narrative_tension">제목은 "답"이 아닌 "질문"을 남겨야 한다. 기법: (1) 미완결 문장("~꺼낸 카드는", "~의 선택") (2) 의외의 대비("A에서 B까지") (3) "왜" 질문 구조. ※ 선언형 종결("~바꾼다", "~이끈다")은 긴장감을 죽이므로 금지.</rule>
</rules>
...
def calculate_title_quality_score(title: str, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if user_keywords:
        ...
        if any_in_front10:
            breakdown['keywordPosition'] = {'score': 20, 'max': 20, 'status': '최적', 'keyword': front_keyword}
        elif any_in_title:
