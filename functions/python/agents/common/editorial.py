...
CONTENT_RULES = {
    ...
    'writingStyle': {
        ...
        'repetitionAvoidance': {
            'principle': '같은 동사, 구문, 슬로건의 과도한 반복은 글의 품격을 심각하게 저하시킴',
            'rules': [
                '동일 단어: 한 문단 내 3회 이하 (조사, 접속사 제외)',
                '유사 구문: 연속된 문장에서 같은 패턴 반복 금지',
                '접속어: "그리고", "또한" 등 남용 지양',
                '동사: 같은 동사를 원고 전체에서 3회 이상 사용 금지 (예: "던지면서" 반복)',
                '슬로건: 캐치프레이즈나 비전 문구는 결론부에서 1회만 사용',
                '벤치마크: "아시아의 싱가포르" 같은 비유는 원고 전체에서 1회만 사용'
            ],
            'alternatives': '다양한 어휘와 문장 구조로 같은 의미 표현',
            'verbDiversity': {
                'principle': '같은 동사를 다른 동의어로 교체하여 글의 다양성 확보',
                'examples': [
                    '"던지면서" → "제시하며", "약속하며", "보여드리며", "열며", "선보이며"',
                    '"이끌어내며" → "달성하며", "만들어내며", "실현하며"',
                    '"살려내다" → "부흥시키다", "재건하다", "일으키다"'
                ],
                'rule': '원고 전체에서 동일 동사 최대 2회 사용, 3회 이상 시 동의어로 교체'
            },
            'sloganRule': {
                'principle': '핵심 슬로건이나 반복 호소문은 결론부 1회로 제한',
                'examples': [
                    '❌ 도입+본론+결론: "청년이 돌아오는 부산" 3회',
                    '✅ 도입: "청년 일자리가 풍부한 부산" / 결론: "청년이 돌아오는 부산" (1회만)'
                ]
            }
        },
        'leadershipTone': {
...
"""
    ...
    'TECH_SAVIOR': {
        'id': 'tech_savior',
        'name': '전문가 권위 전략',
        'profile_condition': lambda p: any(x in (p.get('career', '') or '').lower() for x in ['it', '기업', '대표', 'ceo', '개발', '엔지니어', '박사', '연구']),
        'instruction': """
[수사학 전략: 전문가 권위]
...
def get_active_strategies(topic: str, instructions: str = '', user_profile: dict = {}) -> dict:
    text = f"{topic} {instructions}".lower()
    active_strategies = []
    injections = []

    for key, strategy in RHETORIC_STRATEGIES.items():
        if 'profile_condition' in strategy:
            if strategy['profile_condition'](user_profile):
                active_strategies.append(strategy['id'])
                injections.append(strategy['instruction'])
            continue
        
        if 'keywords' in strategy and any(kw in text for kw in strategy['keywords']):
            active_strategies.append(strategy['id'])
            injections.append(strategy['instruction'])
