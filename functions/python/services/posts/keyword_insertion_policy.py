...
def build_keyword_injection_policy_lines() -> list[str]:
    return [
        ...
        "한 섹션에서는 insert를 최대 1회만 사용하고, 추가 보정은 replace/delete로 처리한다.",
        "결말 섹션(conclusion)에는 insert를 하지 말고 앞 문장을 치환한다.",
        "\"특히/한편/아울러/또한\"으로 시작하는 보강 문장을 연속 추가하지 않는다.",
        "키워드 삽입/치환 후 앞뒤 조사(은/는/이/가/을/를/에/의/로/와/과)가 자연스러운지 반드시 확인한다. 문법이 어색하면 조사를 함께 수정한다. 예) ✗ '인프라도 대해서도' → ✓ '인프라에 대해서도'",
