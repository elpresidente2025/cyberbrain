...
def build_scoring_prompt(
    ...
    if platform_lower == "x":
        platform_specific_rules = """
**X 알고리즘 적합도 체크 (필수):**
- 타래가 2~3개 게시물로 구성되어 있는가? (범위 이탈 시 큰 감점)
- 각 게시물의 본문이 공백·URL 제외 70~110자 범위(권장 95자 내외)인가? 70자 미만 게시물은 큰 감점, 50자 미만은 치명적 감점.
- 첫 게시물의 첫 줄 훅이 강하고 스크롤 정지 효과가 있는가?
- 첫 게시물에 원문 고유명사(인물·지명·단체·사건명) 1개 이상이 포함되는가? (없으면 훅이 공허하다고 판단, 큰 감점)
- 각 게시물이 줄바꿈 카드형(2~5줄)으로 모바일 가독성이 확보되는가?
- 블로그 링크가 타래 마지막 게시물의 마지막 줄에만 1회 포함되는가? (중간 게시물 링크/링크 누락/중복 시 큰 감점)
- 마지막 게시물의 해시태그가 중복 없이 2개 이내인가? (중복/과다 시 감점)
- "자세한 내용은 블로그에서 확인하세요" 같은 저품질 CTA 문구가 없는가? (발견 시 큰 감점)
- 본문에 "정치인 의원님" / "의원님과 함께" 같은 작성자 플레이스홀더 표현이 없는가? (발견 시 치명적 감점)
- 게시물 간 문장/훅 반복이 과도하지 않은가?
- 타래 전반에서 원문의 고유명사/핵심 수치/핵심 주장 중 최소 2개를 보존하는가?
- 길이 평가는 공백·URL 제외 글자 수 기준으로 한다 (마지막 게시물의 블로그 링크·해시태그는 글자 수에 포함되지 않음).
""".strip()
    elif platform_lower == "threads":
        platform_specific_rules = """
**Threads 알고리즘 적합도 체크 (필수):**
- 타래가 3~5개 게시물 범위 안에 있는가? (범위 이탈 시 큰 감점, 4개가 표준)
- 각 게시물이 공백·URL 제외 200~400자 범위에 있는가? (마지막 게시물의 블로그 링크는 글자 수에서 제외)
- 서사 흐름이 "문제의식/공감 → 변화 약속 → 구체 사례 → 정서적 마무리"로 이어지는가?
- 1번 게시물이 자기소개/인사 없이 공감형 훅·문제의식으로 시작하는가? (자기소개 시작 시 큰 감점)
- 변화 약속 역할이 불릿/키워드 나열이 아니라 서술형 문장으로 쓰였는가?
- 구체 사례 게시물에 원문의 고유명사/수치/정책명이 집중 배치되었는가?
- 마지막 게시물이 정서적 마무리 + 블로그 링크로 끝나는가? (링크 누락/중간 게시물 링크/중복 시 큰 감점)
- 해시태그가 마지막 게시물에만 배치되었는가?
- 각 게시물이 독립적으로도 이해 가능하면서 이전/다음과 자연스럽게 이어지는가?
- 게시물 간 문장 반복이 과도하지 않은가?
- 과장/선동형 문구 대신 공감·설명형 문체를 유지하는가?
