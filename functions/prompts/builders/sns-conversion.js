...
/**
 * Facebook/Instagram 공용 단일 게시물용 프롬프트 생성
 * v3.0 - 2026 알고리즘 연구 기반 최적화
 * 
 * 핵심 원칙:
 * - Facebook/Instagram 모두에서 자연스럽게 읽히는 단일 게시물
 * - 첫 2~3줄에 핵심 메시지와 맥락 배치
 * - 한 줄당 20-25자 (모바일 최적화)
 * - 해시태그는 3-5개 이내로 최소화
 * - CTA는 한 문장만 짧게 유지
 */
function buildFacebookInstagramPrompt(cleanContent, platformConfig, userInfo, options = {}) {
  ...
  return `아래는 ${userInfo.name} ${userInfo.position}이 작성한 블로그 원고입니다. 이를 Facebook 게시물과 Instagram 캡션에 모두 사용할 수 있는 단일 SNS 원고로 변환해주세요.
...
## 📱 Facebook/Instagram 공용 최적화 가이드

### [핵심 원칙]
1. **공용 호환성**: Instagram용 과한 해시태그 나열이나 Facebook용 장문 서론 없이, 두 플랫폼 모두에서 자연스럽게 읽히게 하세요.
2. **미리보기 구간 최적화**: 첫 2-3줄에 핵심 메시지와 맥락을 배치해 두 플랫폼 모두에서 초반 이해가 가능해야 합니다.
3. **정보 밀도 유지**: 원고의 고유명사, 수치, 정책명은 살리고 불필요한 수식어는 줄이세요.
4. **구조화된 가독성**: 긴 글도 읽히도록 짧은 호흡의 줄바꿈을 사용하세요.
5. **${charsPerLine}자 호흡**: 모바일 최적화를 위해 짧은 호흡으로 줄바꿈하세요.

### [게시물 구조: 공용 단일 게시물 모드]

**[1] 훅 (첫 125자)**
- 핵심 이슈를 바로 이해하게 하는 강한 첫 문장
- 미리보기에서 끊겨도 맥락이 남는 문장 구조
- 주요 키워드 배치
...
**[3] 상세 설명**
- 블로그 내용을 단순 요약하지 말고, **핵심 배경과 의미를 풀어서 설명**하세요.
- 정책의 배경, 기대효과, 구체적 수치를 포함하세요.
- 이모지는 필요할 때만 최소한으로 사용하세요.

**[4] 진정성 있는 마무리**
- 정치적 구호보다는 개인적인 소회나 비전

**[5] 강력한 CTA**
- 댓글/공감/공유를 유도하되 한 문장 이내로 짧게
...
**✅ 권장:**
- **두 플랫폼에서 모두 읽히는 자연스러운 단일 게시물**로 작성하세요.
- 전문 용어가 있다면 쉽게 풀어서 설명하세요.
- "요약"보다는 "해설" 느낌으로 접근하세요.

**❌ 금지:**
- 인스타그램 전용 해시태그를 과도하게 붙이는 행위
- "자세한 건 블로그에서"라고 퉁치기
- 번호를 붙여 타래처럼 나누는 행위
...
**JSON 출력 형식:**
{
  "content": "변환된 Facebook/Instagram 공용 게시물 전체 텍스트",
...
❌ 느낌표 남발 - 신뢰도 하락
❌ 링크 앞에 별도 안내 문구 - 링크만 마지막 줄에 배치
❌ 블로그 링크를 중간에 넣거나 두 번 이상 반복
...
- 블로그 링크는 마지막 게시물 마지막 줄에 URL만 1회 배치
...
- 원본의 정치적 입장과 논조 완전 보존
- 원본에 없는 내용 추가 금지
- 블로그 링크는 마지막 게시물 마지막 줄에만 1회 배치하고, 별도 안내 문구를 붙이지 말 것
