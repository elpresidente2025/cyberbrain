...
const SEO_RULES = {
  ...
  // 구조 최적화 (황금 비율: 5단 구조, 섹션당 3문단)
  structure: {
    headings: {
      h1: { count: 1, rule: '제목으로만 사용' },
      h2: { count: '3-4개', rule: '서론 제외, 본론 및 결론에 배치' },
      h3: { count: '필요시', rule: '세부 내용 구조화' },
      h4: { count: '필요시', rule: '상세 분류용' }
    },
    paragraphs: {
      count: '15개 내외',
      length: '120-150자',
      rule: '한 섹션당 3문단, 짧고 명확하게'
    },
    lists: {
      usage: '정보 나열 시 적극 활용',
    ...
    contextBasedExceptions: {
      ...
      criticalWriting: {
        ...
        politicalOpponents: {
          ...
          examples: [
            '✅ "윤석열 대통령은 정책 기조를 저버렸습니다" (야당 입장에서 비판)',
      ...
      decisionGuide: {
        ...
        exampleScenarios: [
          ...
          },
          {
            ...
            decision: '비판 맥락 → "윤석열 대통령은 정책 기조를 저버렸습니다" ✅'
          },
          {
...
// ============================================================================
// 소제목 전략 (Subheading Strategies)
// ============================================================================

/**
 * 소제목 작성 전략
 *
 * 목적: 핵심 섹션 1개를 질문형으로 작성하여 AI 검색엔진이
 *       "How/Why 질문에 대한 답변"으로 인식하게 유도
 */
const SUBHEADING_STRATEGIES = {
  // 질문형 및 뉴스 헤드라인형 소제목 전략
  QUESTION_HOOK: {
    id: 'question_hook',
    name: '뉴스 헤드라인/질문형 소제목 전략',
    instruction: `
[소제목(H2) 작성 전략: 뉴스 헤드라인 스타일]

■ 빈도 및 배치 규칙
- **400~500자마다 반드시 소제목(H2)을 삽입**하여 가독성을 높이세요. (2000자 기준 4~5개)
- 긴 문단이 이어지면 중간에 소제목으로 끊어주세요.

■ 소제목 스타일: [뉴스 헤드라인형] 또는 [질문형] 권장
1. **뉴스 헤드라인형 (권장)**: 핵심 키워드와 내용을 명확히 요약
   - ✅ "이관훈 배우, 이재성 후원회장 전격 합류"
   - ✅ "부산 3대 공약, 어떻게 실현되나"
   - ✅ "이재성 후보, 부산 경제의 새로운 대안 제시"
...
■ 나쁜 예 (절대 금지)
- ❌ 추상적인 단어만 사용: "서론", "본론", "결론", "들어가며", "마치며" (X)
- ❌ 단순 나열: "첫째", "둘째", "세 번째" (X - 뒤에 내용이 있어야 함)
- ❌ 너무 긴 문장: 소제목은 한 눈에 들어오게 20자 이내로 작성

■ 구조 예시
<h2>이관훈, 이재성 후원회장 맡기로</h2>
(내용...)
<h2>이재성과 이관훈이 꿈꾸는 부산의 미래는?</h2>
(내용...)
<h2>이관훈 후원회장, 부산 시민 희망 제시</h2>
(내용...)
<h2>함께 만들어갈 부산의 미래</h2>
(마무리...)

■ 나열 구조 소제목 규칙
- "첫째, 둘째, 셋째" 나열 시에도 반드시 **[접속사 + 핵심내용]** 형태로 소제목을 만드세요.
- 예: <h2>첫째, 다대포 디즈니랜드 조성</h2>
`
  }
};
...
/**
 * 소제목 작성 가이드라인 반환
 * @returns {string} 프롬프트에 주입할 소제목 전략 문자열
 */
function getSubheadingGuideline() {
  return SUBHEADING_STRATEGIES.QUESTION_HOOK.instruction;
}
