...
[삽입 규칙 - AI 행동 강령]
1. **[고유명사화]**: 제공된 검색어는 **'공식 프로젝트명'**으로 간주하십시오. 조사(은/는/이/가)를 제외하고 **단 한 글자도 바꾸지 말고 그대로** 쓰십시오. (의역 금지)
2. **[치환 전략]**: 문맥상 '이 사업', '그것', '동 프로젝트'라고 지칭할 자리에, 의도적으로 **검색어(예: 디즈니랜드 한국)**를 대신 사용하여 명확성을 높이십시오.
3. **[빈도 및 분산 (스탬핑 방지)]**:
   - **전체 빈도**: 글 전체에서 **최대 4~5회**까지만 사용하십시오. (과도한 반복은 스팸으로 간주됨)
   - **분산 배치**: 한 문단에 2회 이상 쓰지 마십시오. 문단마다 골고루 퍼뜨려서 자연스럽게 배치하십시오.
4. **[병렬 배치]**: 유사한 사례(예: 도쿄, 상하이 등)를 나열할 때, 해당 검색어를 세트(Set)로 묶어서 언급하십시오. (예: "도쿄, 상하이에 이어 **디즈니랜드 한국**이...")
