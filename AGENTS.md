...
- 한국어 어휘·표현 필터링이 필요할 때는 원시 문자열/정규식 휴리스틱만으로 차단하지 말고, 가능한 경우 Kiwi 형태소 분석을 사용한다. 정규식은 빠른 후보 탐색이나 Kiwi 미사용 환경의 fallback으로만 두고, 최종 차단·분류는 품사/어절/문장 경계 등 형태소 기반 근거를 우선한다.
